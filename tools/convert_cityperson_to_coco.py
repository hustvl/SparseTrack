...
DATA_PATH = '/data/zelinliu/Citypersons/'
...
if __name__ == '__main__':
    ...
    for img_path, label_path in zip(img_paths, label_paths):
        image_cnt += 1
        im = Image.open(os.path.join("/data/zelinliu/", img_path))
        ...
        # Load labels
        if os.path.isfile(os.path.join("/data/zelinliu/", label_path)):
            labels0 = np.loadtxt(os.path.join("/data/zelinliu/", label_path), dtype=np.float32).reshape(-1, 6)
