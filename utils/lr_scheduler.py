...
class LRHook(HookBase):
    def __init__(
        self, 
        trainer, train_batch_size, basic_lr_per_img,
        scheduler_name, iters_per_epoch, max_eps, num_warmup_eps, 
        warmup_lr_start, no_aug_eps, min_lr_ratio
    ):
        ...
        self.num_warmup_eps = num_warmup_eps
        self.warmup_lr_start = warmup_lr_start
        self.no_aug_eps = no_aug_eps
        self.min_lr_ratio = min_lr_ratio
   
    def after_step(self):
        lr = self.lr_scheduler.update_lr(self._trainer.iter + 1)###
        for param_group in self._trainer.optimizer.param_groups:
            param_group["lr"] = lr
        self._trainer.storage.put_scalar("lr", lr, smoothing_hint=False)
    
    def before_train(self):
        self.lr_scheduler = LRScheduler(
            name = self.scheduler_name,
            lr = self.basic_lr_per_img * self.train_batch_size, 
            iters_per_epoch = self.iters_per_epoch,
            total_epochs = self.max_eps,
            warmup_epochs = self.num_warmup_eps,
            warmup_lr_start = self.warmup_lr_start,
            no_aug_epochs = self.no_aug_eps,
